...
- The uv and Claude Code installers now download in parallel with the package phase: a `bootcmd`-launched prefetch script runs them as soon as the network and user exist, and `runcmd` waits on a completion marker (falling back to inline installs if the prefetch didn't finish — both installers are idempotent). This hides the installers' ~60-70s behind the package download instead of adding it after. Lock safety: both installers are pure downloads into `~/.local` and never invoke the package manager; as belt-and-braces the apt tuning sets `DPkg::Lock::Timeout` (dnf already blocks on its lock), and on wait timeout runcmd kills the prefetch process group before falling back so the two install paths never run concurrently.
