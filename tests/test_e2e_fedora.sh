...
setup_e2e() {
    E2E_DIR="$(mktemp -d "${TMPDIR:-/tmp}/claude-vm-e2e-fedora-XXXXXX")"
    export CLAUDE_VM_DIR="$E2E_DIR/data"
    export SSH_PORT_BASE=17022
    export FLAVOR=fedora-slim
    export CLAUDE_VM_QUIET=true

    FAKE_PROJECT_A="$E2E_DIR/project-fedora-a"
    FAKE_PROJECT_B="$E2E_DIR/project-fedora-b"
    mkdir -p "$FAKE_PROJECT_A" "$FAKE_PROJECT_B"
    echo "hello from fedora project A" > "$FAKE_PROJECT_A/testfile.txt"
    echo "hello from fedora project B" > "$FAKE_PROJECT_B/testfile.txt"
}
...
cleanup_e2e() {
    ...
    pkill -f "claude-vm-e2e-fedora" 2>/dev/null || true
    sleep 1
...
phase_build() {
    ...
    local output
    if output=$(timeout 600 bash "$CLAUDE_VM" build --flavor fedora-slim 2>&1); then
        local base_img="$CLAUDE_VM_DIR/base/base-fedora-slim.qcow2"
    ...
    local start_time
    start_time=$(date +%s)
    output=$(timeout 30 bash "$CLAUDE_VM" build --flavor fedora-slim 2>&1) || true
...
phase_launch() {
    ...
    base_img="$CLAUDE_VM_DIR/base/base-fedora-slim.qcow2"
    ...
    if [[ "$distro_id" == "fedora" ]]; then
        pass "guest is Fedora"
    else
        fail "guest is Fedora" "got ID=$distro_id"
    fi
    ...
    if [[ "$guest_content" == "hello from fedora project A" ]]; then
        pass "virtiofs read: host file readable from guest"
    else
        fail "virtiofs read" "expected 'hello from fedora project A', got '$guest_content'"
    fi
    ...
    if (( ${#missing_pkgs[@]} == 0 )); then
        pass "slim packages installed (rpm -q)"
    else
        fail "slim packages installed" "missing: ${missing_pkgs[*]}"
    fi

    # Test: npm present (dnf resolves the npm name to the nodejs-npm rpm)
    if _e2e_ssh "$FAKE_PROJECT_A" "command -v npm" &>/dev/null; then
        pass "npm available (nodejs-npm)"
    else
        fail "npm available" "command -v npm failed"
    fi

    # Test: full-only packages are NOT in the slim image
    if ! _e2e_ssh "$FAKE_PROJECT_A" "rpm -q tmux" &>/dev/null; then
    ...
    if _e2e_ssh "$FAKE_PROJECT_A" "command -v dnf" &>/dev/null; then
        pass "dnf available in guest"
    else
        fail "dnf available in guest" "not found"
    fi
...
phase_resume() {
    ...
    if [[ "$content" == "hello from fedora project A" ]]; then
...
main() {
    ...
    echo "=== claude-vm E2E tests (fedora) ==="
    ...
    echo ""
    echo "=== Results (fedora): $TESTS_PASSED passed, $TESTS_FAILED failed, $TESTS_SKIPPED skipped, $TESTS_RUN total ==="
