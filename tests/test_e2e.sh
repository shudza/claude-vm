...
phase_launch() {
    ...
    if _e2e_ssh "$FAKE_PROJECT_A" "claude --version" &>/dev/null; then
        pass "Claude Code installed in VM"
    else
        # Non-fatal — claude may not be installed if build used a cached/old base image
        skip "Claude Code not found in VM (may need base image rebuild)"
    fi
}
